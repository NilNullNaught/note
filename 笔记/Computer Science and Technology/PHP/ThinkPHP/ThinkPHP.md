...
- 搜索器是用于封装字段（或搜索标识）的查询表达式，类似于查询范围方法。
...
- ###### 创建搜索器
  ...
  一个搜索器对应模型的一个特殊方法，该方法为 `public`。方法名的命名规范为 `searchFieldAttr()`。

- ###### 调用搜索器

  1. 使用静态方法 `::withSearch()` 调用搜索器，该方法可以同时调用多个搜索器。
  2. withSearch() 的第一个参数是一个数组，用于指定使用的搜索器；第二个是用于指定搜索器的参数的表达式值；第三个参数$data，可以得到 withSearch() 方法第二参数的值。
  ...
  class TestController{
      public function index(){
  		$result = UserModel::withSearch(['email', 'create_time'],
                                          ['email' => 'xiao','create_time' 
                                            => ['2014-1-1', '2017-1-1']])->select();
      }
  }
  ```
...
- 模型的数据集也是直接继承 collection 类，使用数据集对象和数组操作方法一样，都可以循环遍历、删除元素等。
...
| hidden()  | 隐藏某个字段                                                 |
| visible() | 只显示某个字段                                               |
| append()  | 添加某个获取器字段，然后可以使用 withAttr() 对字段进行函数处理； |
...
- 在需要开启自动时间戳的模型中设置字段 `$autoWriteTimestamp`。

  ```php
  //开启自动时间戳
  protected $autoWriteTimestamp = true;
  
  //关闭自动时间戳（开启全局自动时间戳后，对单个模型进行关闭）
...
##### 📌自动时间戳只在模型下有效，如果直接调用数据库方法将无法时间自动时间戳

<br>

##### 📌如果时间字段的字段名不是 create_time 和 update_time

- 如果时间字段的字段名不是默认的 create_time 和 update_time，也可以进行自定义
...
##### 关闭 create_time 或 update_time 的自动时间戳功能

- ###### 在模型中设置

  如果业务中只需要 `create_time` 而不需要 `update_time`，可以关闭它，同理也可以关闭 `create_time`。
...
- ###### 动态实现
  ...
  可以动态实现不修改 `update_time` 或 `create_time` ：
...
#####  使用关联查询后，进一步对查询结果进行筛选

- ###### 说明

  使用 `->[附表名]()` 可以返回  `$query` 对象，并对查询结果进行进一步的筛选。
...
- ###### 说明
  ...
  使用 `together()` 可以在删除主表内容时，将附表关联的内容全部删除；
